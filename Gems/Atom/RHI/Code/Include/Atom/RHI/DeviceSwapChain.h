...
namespace AZ::RHI
{
    ...
    {
    public:
        AZ_RTTI(DeviceSwapChain, "{888B64A5-D956-406F-9C33-CF6A54FC41B0}", Object);
        virtual ~DeviceSwapChain();
        ...
        DeviceImage* GetCurrentImage() const;

        //! Returns the image associated with the provided index, where the total number of images
        //! is given by GetImageCount().
        DeviceImage* GetImage(uint32_t index) const;

        //! Returns the ID used for the DeviceSwapChain's attachment
    ...
    private:
        ...
        AZStd::vector<Ptr<DeviceImage>> m_images;
